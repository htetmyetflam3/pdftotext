...
export const PdfDemoOverlay: React.FC<PdfDemoOverlayProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-slate-950/80 backdrop-blur-md overflow-y-auto animate-in fade-in duration-300">
      {/* Two Frame Modal Container */}
      <div className="relative w-full max-w-6xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden my-auto flex flex-col max-h-[92vh]">
        {/* Top Header bar with status and Close button */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="font-bold text-base sm:text-lg">Zawgyi PDF Deep Parser Engine</h3>
                <span className="hidden sm:inline-block px-2 py-0.5 text-[10px] font-semibold bg-emerald-500/20 text-emerald-400 rounded-full border border-emerald-500/30">
                  Dual-Engine Encoding Solver
                </span>
              </div>
              <p className="text-xs text-slate-400">
                {customFileUploaded ? `Custom File: ${customFileUploaded}` : `Sample PDF: ${sampleDoc.title} (${sampleDoc.size})`}
              </p>
            </div>
          </div>
        ...
        <div className="bg-slate-100/90 border-b border-slate-200 px-6 py-2.5 flex items-center justify-between text-xs flex-wrap gap-2">
          <div className="flex items-center gap-2">
        ...
        {parsingStep !== 'ready' && (
          <div className="p-8 text-center bg-sky-50/50 flex flex-col items-center justify-center">
